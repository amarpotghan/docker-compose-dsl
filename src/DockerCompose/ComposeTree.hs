...
newtype DockerCompose m a
  = DockerCompose {
    runDockerCompose :: StateT ComposeTree m a
    }
  deriving (Functor, Applicative, Monad, MonadState ComposeTree)
...
appendProperty p = onLast addProp
  where
    addProp cn = cn { nodeProperties = (nodeProperties cn) ++ [ p ] }

appendValue :: ValueRep -> ComposeTree -> ComposeTree
appendValue v = onLast (\ cn -> cn { nodeProperties = onLast addVal (nodeProperties cn) } )
  where
    addVal (PlainList k vs) = PlainList k (vs ++ [ v ])
    addVal x = x

onLast :: (a -> a) -> [ a ] -> [ a ]
onLast f = L.reverse . go . L.reverse
  where
    go (x : xs) = f x : xs
    go [] = []
