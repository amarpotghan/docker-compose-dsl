...
instance (KnownSymbol a, Show value, ToStringy s) =>
  HasGenerator (s := Capture a value) where
  type Arg (s := Capture a value) = value
  generate _ v = modify $ appendProperty $ SingleVal k value
    where
...
instance (KnownSymbol a, ToStringy s) => HasGenerator (s := a) where
  type Arg (s := a) = NoArg
  generate _ _ = modify $ appendProperty (SingleVal k v)
    where
      k = KeyRep . L.pack $ toStringy (Proxy :: Proxy s)
...
instance ( Show a, ToStringy s) => HasGenerator (s := Values a) where
  ...
  generate _ v = modify $ appendProperty $ PlainList k values
    where
      k = KeyRep $ L.pack (toStringy (Proxy :: Proxy s))
...
instance (KnownSymbol a, ToStringy s, HasGenerator rest) => HasGenerator s := (a ': rest) where
   ...
   generate _ v =
     (modify $ appendProperty $ PlainList k [])
